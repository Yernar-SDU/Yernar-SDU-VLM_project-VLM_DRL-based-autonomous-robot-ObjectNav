...
class ActorSAC(nn.Module):
    ...
    def __init__(self, action_dim=2, log_std_min=-20, log_std_max=2):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=4, stride=2, padding=1),  # 32 x 32 x 32
            nn.ReLU(),
    ...
    def forward(self, img, scalars):
        """
        Pass inputs through the network to get mean and log_std.
        """
        c = self.conv(img)
        c = c.view(c.size(0), -1)
    ...
    def sample(self, img, scalars):
        """
        Returns:
            action: tanh-squashed action sample
            log_prob: log probability of the action
            mean: mean action (before squashing)
        """
        ...
        # To compute log_prob, account for tanh squashing
...
class SAC(object):
    def __init__(self, action_dim=2, max_action=1.0, discount=0.99, tau=0.005,
        ...
        # Temperature parameter for entropy regularization (log_alpha)
        # If target_entropy is not provided, we use -action_dim as a heuristic.
    ...
    def get_action(self, img_np, scl_np, evaluate=False):
        """
        For evaluation, return the deterministic action (tanh of mean).
        ...
        with torch.no_grad():
            ...
            if evaluate:
                # Use tanh(mean) as deterministic action
                action = torch.tanh(mean)
        action = action.cpu().numpy().flatten()
        # Scale action if necessary
    ...
    def train(self, replay_buffer, iterations, batch_size=128, beta=0.4):
        ...
        for it in range(iterations):
            ...
            batch_img      = torch.from_numpy(batch_img).to(device)          # (batch,1,64,64)
            batch_scl      = torch.from_numpy(batch_scl).to(device)          # (batch,7)
            batch_actions  = torch.from_numpy(batch_actions).to(device)      # (batch, action_dim)
            batch_rewards  = torch.from_numpy(batch_rewards).unsqueeze(-1).to(device)  # (batch,1)
            batch_dones    = torch.from_numpy(batch_dones).unsqueeze(-1).to(device)    # (batch,1)
            batch_next_img = torch.from_numpy(batch_next_img).to(device)     # (batch,1,64,64)
            batch_next_scl = torch.from_numpy(batch_next_scl).to(device)     # (batch,7)
            weights        = torch.from_numpy(weights).unsqueeze(-1).to(device)  # (batch,1)

            # ---------------------- Update Critic ---------------------- #
            with torch.no_grad():
                # Sample action from target policy for next state
            ...
            # Get current Q estimates from critic networks
            current_Q1, current_Q2 = self.critic(batch_img, batch_scl, batch_actions)
            # Compute critic losses (MSE weighted by PER importance sampling weights)
            ...
            # Update priorities in PER buffer using TD error (absolute error)
            with torch.no_grad():
            ...
            # Evaluate Q for new actions
            q1_new, q2_new = self.critic(batch_img, batch_scl, action_new)
            q_new = torch.min(q1_new, q2_new)
            # Actor loss: maximize Q - alpha * log_prob (or minimize the negative)
...
def evaluate(network, env, epoch, eval_episodes=5):
    ...
    for _ in range(eval_episodes):
        ...
        while not done and steps < 500:
            ...
            # Scale the action if needed (example: mapping from [-1,1] to required range)
...
if __name__ == "__main__":
    ...
    # Assume GazeboEnv is defined in realsense_env.py
    ...
    # Assume PrioritizedReplayBuffer is imported from per_replay_buffer.py
    ...
    # Exploration settings (if needed for initial random actions)
    expl_noise = 1.0
