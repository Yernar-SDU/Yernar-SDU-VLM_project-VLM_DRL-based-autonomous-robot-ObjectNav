...
# For the depth image shape (width, height)
IMG_HEIGHT = 64
IMG_WIDTH = 64
...
class GazeboEnv:
    """
    This environment now returns a SINGLE depth-based channel of shape (1,64,64)
    plus a 7D array of scalars [prev_lin, prev_ang, last_lin, last_ang, dist2goal, angle2goal, min_laser].
    """
    ...
    def __init__(self):
        ...
        # Distances from the RealSense depth array
    ...
    def depth_callback(self, msg):
        """
        We store a 1-channel normalized depth image in self.normed_depth,
        plus the min depth for collision detection / reward logic.
        """
        try:
        ...
        # Build the 20-beam array for collision detection
        ...
        # The min scaled value => real distance
    ...
    def step(self, action):
        ...
        # dark blue => forced collision
        if np.any(self.realsense_data < BLUE_DISTANCE_THRESHOLD):
            print("[Env] Dark blue => extremely close => reset!")
            done = True
            collision = True
        ...
        (base_r, prog_r, wall1, angle_b, wall2, final_r) = self.get_reward(
        ...
        # debug
        print(f"[REWARD DETAILS] base={base_r:.2f}, progress={prog_r:.2f}, "
              f"wall1={wall1:.2f}, angle={angle_b:.2f}, wall2={wall2:.2f}, total={final_r:.2f}")
        ...
        # The single-channel (64x64) depth image
    ...
    def reset(self):
        ...
        # Possibly no depth callback yet
        distance = np.linalg.norm([self.odom_x - self.goal_x, self.odom_y - self.goal_y])
        if not hasattr(self, 'min_wall_real'):
            self.min_wall_real = 10.0
        self.prev_distance = distance
    ...
    def check_pos(self, x, y):
        # same obstacle checks as before
    ...
    def get_reward(
        ...
        """
        Using the real scaled distances for the wall penalty.
        """
        current_scaled = current_wall_real
        prev_scaled = prev_wall_real
        ...
        if target:
            ...
            # 2) progress term
            progress = K * (prev_distance - distance)

            # 3) wall1 penalty if <2.5
            wall1 = 0.0
            if current_scaled < 2.5:
                wall1 = -(2.5 - current_scaled)
            ...
            # 5) wall2 penalty
            wall2 = 0.0
            dist_decreased = (prev_scaled - current_scaled)
            if dist_decreased > 0:
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        # random action
        action = [random.uniform(0.0, 0.5), random.uniform(-1.0, 1.0)]
