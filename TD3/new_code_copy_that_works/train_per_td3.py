...
class CriticCNN(nn.Module):
    ...
    Each Q-network has its own conv+fc or can share. We'll do separate for maximum independence.
    """
    ...
    def __init__(self, action_dim=2):
        ...
        # we also merge the action => final dimension = conv_size1 + 64 + action_dim
...
class TD3(object):
    ...
    def get_action(self, img_np, scl_np):
        """
        img_np: (1,64,64) in np.float32
        scl_np: (7,) in np.float32
        We'll feed them to the actor.
        """
    ...
    def train(
        ...
        for it in range(iterations):
            ...
            if it % policy_freq == 0:
                ...
                actor_Q1, actor_Q2 = self.critic(batch_img, batch_scl, actor_actions)
...
def evaluate(network, env, epoch, eval_episodes=5):
    ...
    for _ in range(eval_episodes):
        ...
        while not done and steps < 500:
            ...
            # Scale it for your environment if needed
            # For example, turn [-1,1] into [0..1] for linear velocity, etc.
...
if __name__ == "__main__":
    ...
    save_model = True
    file_name = "TD3_PER_CNN_singlechannel"
    batch_size = 64
    discount = 0.99
    tau = 0.005
    policy_noise = 0.2
    ...
    while timestep < max_timesteps:
        ...
        # Scale for env if needed
        a_in = [(action[0] + 1)/2, action[1]]  # e.g. linear in [0..1], angular in [-1..1]
