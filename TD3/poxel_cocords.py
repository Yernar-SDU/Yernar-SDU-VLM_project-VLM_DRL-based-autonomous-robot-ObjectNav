...
class VLM_Response_Processor:
    ...
    def __init__(self, save_dir="/tmp/exploration_data"):
        self.save_dir = save_dir
        self.bridge = CvBridge()

        # Camera calibration
        self.camera_to_robot_translation = np.array([0.219, 0.02, 0.32])
        ...
        world_file = "/home/ai-lab/Downloads/DRL-robot-navigation_segway_imu_should_be_calibrated/DRL-robot-navigation/catkin_ws/src/multi_robot_scenario/launch/TD3_many.world"
        ...
        # self.owlDetector = OwlVLMDetector(text_queries=["a red fire hydrant", "a blue box on the floor", "a table with grey legs", "a bookshelf with 3 shelves"])
    ...
    def detect_objects(self, cv_image, pointcloud, pose, timestamp, frame_id):
        ...
        try:
            ...
            # detected_objects = self.owlDetector.detect(cv_image)
            ...
            for obj in detected_objects:
                world_coords = self.get_coords_from_pointcloud(
                    obj['center_x'], obj['center_y'], pointcloud, pose
                )
    ...
    def get_coords_from_pointcloud(self, pixel_x, pixel_y, pointcloud, current_pose):
        ...
        # Step 1: Force to integer and stay within camera bounds
        # This prevents the "integer argument expected" error
        ...
        try:
            for dx in range(-search_radius, search_radius + 1):
                for dy in range(-search_radius, search_radius + 1):
                    ...
                    # Check bounds before calling read_points
                    if 0 <= u < width and 0 <= v < height:
                        # uvs MUST be a list of (int, int)
                        ...
                        if points:
                            x, y, z = points[0]
                            # Ensure the data is finite (not Inf or NaN)
            ...
            # Step 2: Robust estimation using Median
            samples_array = np.array(samples)
            cam_x, cam_y, cam_z = np.median(samples_array, axis=0)
            print("cam_x, cam_y, cam_z", cam_x, cam_y, cam_z)
            
            # Step 3: Transformation
            # Your current function uses: point_robot_frame = np.array([cam_z, -cam_x, -cam_y])
            # This is correct for most ROS optical-to-base links.
    ...
    def save_object_detection(self, obj, world_coords, timestamp, current_pose, frame_id):
        """
        Update object position ONLY by name.
        ...
        for existing in self.detected_objects.values():
            if existing["object_class"] == obj_class:
                existing["world_x"] = world_coords[0]
                existing["world_y"] = world_coords[1]
                existing["world_z"] = world_coords[2]
